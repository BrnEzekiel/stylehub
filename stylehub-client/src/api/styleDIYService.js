...
export const getStyleDIYPosts = async (params = {}) => {
  try {
    const response = await apiClient.get('/style-diy', { params });
...
export const addStyleDIYComment = async (postId, content) => {
  try {
    const response = await apiClient.post(`/style-diy/${postId}/comments`, { content });
    return response.data;
  } catch (error) {
