...
import { getAdminStayById, adminUpdateStay, deleteStay } from '../api/adminService';
...
function EditStayPage() {
  ...
  useEffect(() => {
    const fetchStay = async () => {
      try {
        setLoading(true);
        const data = await getAdminStayById(id);
        setFormData({
          name: data.name,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await adminUpdateStay(id, data);
      setSuccess('Stay updated successfully!');
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to permanently delete this stay?')) {
      try {
        ...
        alert(`Failed to delete stay: ${err.message}`);
        setError(err.message);
        setLoading(false);
      }
    }
  ...
  if (loading && !error) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', color: 'white' }}>
        ...
        <h1 style={{ marginLeft: '20px' }}>Loading Stay...</h1>
    </div>
    );
  }
          ...
          <div>
            ...
            {currentImageUrl && (
              <div style={{ marginBottom: '16px' }}>
                ...
                <img src={currentImageUrl} alt="Current Stay" style={{ maxWidth: '100%', height: '100px', objectFit: 'contain' }} />
              </div>
            )}
            <input
              type="file"
