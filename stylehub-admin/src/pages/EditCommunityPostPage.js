...
import { getAdminCommunityPostById, adminUpdateCommunityPost, deleteCommunityPost } from '../api/adminService';
import Page from '../components/Page';
import { FaArrowLeft, FaTrash } from 'react-icons/fa';

function EditCommunityPostPage() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        setLoading(true);
        const data = await getAdminCommunityPostById(id);
        setFormData({
          title: data.title,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await adminUpdateCommunityPost(id, data);
      setSuccess('Community post updated successfully!');
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to permanently delete this community post?')) {
      try {
        ...
        alert(`Failed to delete community post: ${err.message}`);
        setError(err.message);
        setLoading(false);
      }
    }
  ...
  if (loading && !error) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', color: 'white' }}>
        ...
        <h1 style={{ marginLeft: '20px' }}>Loading Community Post...</h1>
    </div>
    );
  }
  ...
  return (
    <Page title="Edit Community Post">
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            {currentImageUrl && (
              <div style={{ marginBottom: '16px' }}>
                ...
                <img src={currentImageUrl} alt="Current Community Post" style={{ maxWidth: '100%', height: '100px', objectFit: 'contain' }} />
              </div>
            )}
            <input
              type="file"
          ...
          <button type="submit" disabled={loading} style={{
            ...
            {loading ? 'Updating...' : 'Update Community Post'}
          </button>
          <button
          ...
          >
            {loading ? 'Deleting...' : 'Delete Community Post'}
          </button>
        </form>
      </div>
    </Page>
